...
- 游戏：MARBLES
- 类型：casual
- 简述：重力沙盒 — 同色合并爆裂、连击奖励彩虹雨，倾斜改重力、摇晃即弹跳
- 框架 / 语言 / 构建：React, TypeScript, Vite, Less
- 渲染方式：Canvas/WebGL
- 依赖摘录：@types/react@^18.3.12, @types/react-dom@^18.3.1, @vitejs/plugin-react@^4.3.4, less@^4.2.0, react@^18.3.1, react-dom@^18.3.1, typescript@^5.6.3, vite@^5.4.21
- 平台元信息：meta.title=MARBLES；cover_url=/poster.png；category=casual；uuid=399ef6c5-aa3f-437a-96a7-52bd002aa59e
...
- `src/Marbles/Marbles.less`：视觉样式、布局、动画和响应式规则。
- `src/Marbles/Marbles.tsx`：React 组件和交互界面。
- `src/Marbles/index.ts`：游戏源码模块。
...
- `src/App.less`
- `src/game-id.ts`
- `src/Marbles/Marbles.less`
- `src/Marbles/Marbles.tsx`
- `src/Marbles/index.ts`
